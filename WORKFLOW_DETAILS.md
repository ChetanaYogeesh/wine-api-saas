...
Tests use SQLite in-memory database for speed and isolation. The `conftest.py` sets `DATABASE_URL` before importing app modules:
...
This ensures all app code (including middleware using `SessionLocal`) uses the test database.
...
1. **Lint** - ruff, black
2. **Test** - pytest with SQLite (no PostgreSQL/Redis services needed)
3. **Build** - Docker multi-platform
4. **Push** - GitHub Container Registry
5. **Deploy** - Railway/Heroku (on tags)

**Environment:** `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` set for Node.js 24 compatibility
