...
**Test Configuration:** Tests use SQLite in-memory database for speed and isolation. The `conftest.py` sets `DATABASE_URL` before importing app modules to ensure all app code (including middleware) uses the test database.
...
1. Lint (ruff, black)
2. Test (SQLite - no PostgreSQL/Redis needed)
